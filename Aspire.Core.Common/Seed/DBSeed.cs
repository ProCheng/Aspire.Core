using Aspire.Core.Common.Const;
...
namespace Aspire.Core.Common.Seed
{
    public class DBSeed
    {
        private static string SeedDataFolder = "AspireCore.Data.json/{0}.tsv";
        ...
        public static async Task SeedAsync(MyContext myContext, string WebRootPath)
        {
            try
            {
                ...
                Console.WriteLine("************ Aspire.Core DataBase Set *****************");
                ...
                var referencedAssemblies = System.IO.Directory.GetFiles(path, "Aspire.Core.Model.dll")
                    .Select(Assembly.LoadFrom).ToArray();
                var modelTypes = referencedAssemblies
                    .SelectMany(a => a.DefinedTypes)
                    .Select(type => type.AsType())
                    .Where(x => x.IsClass && x.Namespace is "Aspire.Core.Model.Models")
                ...
                if (AppSettings.app(new string[] { "AppSettings", "SeedDBDataEnabled" }).ObjToBool())
                {
                    ...
                    #region AspireArticle

                    if (!await myContext.Db.Queryable<AspireArticle>().AnyAsync())
                    {
                        myContext.GetEntityDB<AspireArticle>().InsertRange(
                            JsonHelper.ParseFormByJson<List<AspireArticle>>(
                                FileHelper.ReadFile(string.Format(SeedDataFolder, "AspireArticle"), Encoding.UTF8)));
                        Console.WriteLine("Table:AspireArticle created success!");
                    }
                    else
                    {
                        Console.WriteLine("Table:AspireArticle already exists...");
                    }
                    ...
                    if (!await myContext.Db.Queryable<Role>().AnyAsync())
                    {
                        ...
                        //using var stream = new FileStream(Path.Combine(WebRootPath, "AspireCore.Data.excel", "Role.xlsx"), FileMode.Open);
            ...
            {
                throw new Exception(
                    $"1、若是Mysql,查看常见问题:https://github.com/anjoy8/Aspire.Core/issues/148#issue-776281770 \n" +
                    $"2、若是Oracle,查看常见问题:https://github.com/anjoy8/Aspire.Core/issues/148#issuecomment-752340231 \n" +
                    "3、其他错误：" + ex.Message);
            }
        }
        ...
        public static void MigrationLogs(MyContext myContext)
        {
            ...
            var modelTypes = referencedAssemblies
                .SelectMany(a => a.DefinedTypes)
                .Select(type => type.AsType())
                .Where(x => x.IsClass && x.Namespace != null && x.Namespace.StartsWith("Aspire.Core.Model.Logs"))
                .ToList();
