...
#注意下容器内输出端口是9291
#如果你想先手动dotnet build成可执行的二进制文件，然后再构建镜像，请看层下的dockerfile。
...
WORKDIR /src
COPY ["Aspire.Core/Aspire.Core.csproj", "Aspire.Core/"]
...
COPY ["build", "build/"]
RUN dotnet restore "Aspire.Core/Aspire.Core.csproj"
COPY . .
WORKDIR "/src/Aspire.Core"
RUN dotnet build "Aspire.Core.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Aspire.Core.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
EXPOSE 9291 
ENTRYPOINT ["dotnet", "Aspire.Core.dll"]
