...
public class DynamicLambdaTest
{
    ...
    private void Init()
    {
        _baseRepository.Db.CodeFirst.InitTables<AspireArticle>();
        _baseRepository.Db.CodeFirst.InitTables<AspireArticleComment>();
    ...
    /// </summary>
    [Fact]
    public async void Get_Aspires_DynamicTest()
    {
        //方便前端自定义条件查询 
        //语法更舒服
    ...
    /// </summary>
    [Fact]
    public async void Get_Aspires_DynamicJoinTest()
    {
        //方便前端自定义条件查询 
        //语法更舒服
        var data = await _baseRepository.Query();
    ...
    {
        var express = DynamicLinqFactory.CreateLambda<AspireArticle>(conditions);
    ...
    private async Task TestJoinConditions(string conditions)
    {
        var express = DynamicLinqFactory.CreateLambda<AspireArticle>(conditions);
        _testOutputHelper.WriteLine(new string('=', 100));
        var product = await _baseRepository.Db.Queryable<AspireArticle>()
